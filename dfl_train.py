...
class DFL_Surrogate_Function(torch.autograd.Function):
    ...
    def forward(ctx, pred_ata_tensor, true_ata_tensor, instance, config):
        ...
        if grad_norm > 50.0:
            grad_pred_ata = grad_pred_ata / grad_norm * 50.0
...
def train_dfl(prefix, num_flights=20):
    ...
    for epoch in range(10000):
        ...
        for i, day in enumerate(days_train):            
            ...
            # 域随机化
            k_sample = np.random.randint(8, 13)
            config = create_dynamic_config(k_sample)
            
            raw = day_df[['feat_weather', 'buffer', 'interval_next']].values
            aug = augment_features(raw, k_sample, num_flights)
            ...
            loss = DFL_Surrogate_Function.apply(pred_ata_tensor, true_ata, instance, config)
            (loss / (batch_size * accum_steps)).backward()            
            epoch_train_surr += loss.item()

            if (i + 1) % batch_size == 0 or (i + 1) == len(days_train):
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)
                if ((i + 1) // batch_size) % accum_steps == 0 or (i + 1) == len(days_train):
                    optimizer.step()
                    optimizer.zero_grad()
        ...
        val_k = 10 
        val_config = create_dynamic_config(val_k)
        ...
        with torch.no_grad():
            for day in days_val:
                ...
                raw = day_df[['feat_weather', 'buffer', 'interval_next']].values
                aug = augment_features(raw, val_k, num_flights)
                norm = (aug - global_mean) / global_std
                ...
                solver = GSESolver(val_config)
                model_mip, vars_mip, *_ = solver.build_model(instance, pred_ata, relax=False)
                model_mip.setParam("OutputFlag", 0)
                model_mip.setParam("MIPGap", 0.05)
                model_mip.optimize()
                ...
                if model_mip.SolCount > 0:
                    ...
                    model_3.optimize()
                    epoch_val_regret += model_3.ObjVal
                else:
                    epoch_val_regret += 100000.0
        ...
        print(f"Epoch [{epoch+1:03d}] | Tr Surr: {epoch_train_surr/len(days_train):.1f} | Val Regret(K=10): {avg_val_regret:.1f}")
    ...
    print(f"DFL 模型就绪。最佳 Regret: {best_val_regret:.2f}")
